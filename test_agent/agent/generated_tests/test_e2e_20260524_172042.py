...
def test_e2e_equivalence(tmp_path):
    ...
    with patch("urllib.request.urlopen", return_value=mock_resp):
        orig_scraper = OriginalConversationScraper(conv_id, offset, chunk_size, cookie, fb_dtsg, user_id, out_dir / "orig")
        orig_scraper.scrapeConversation(False)

    with responses.RequestsMock() as rsps:
        rsps.add(responses.POST, 'https://www.facebook.com/ajax/mercury/thread_info.php',
                 json=RESPONSE_DATA, status=200)

        mig_scraper = MigratedConversationScraper(conv_id, cookie, fb_dtsg, out_dir / "mig")
        mig_scraper.scrapeConversation(False, 0, 0, 2000, 0, False)

    orig_result = json.loads((out_dir / "orig" / conv_id / "conversation.json").read_text())
    mig_result = json.loads((out_dir / "mig" / conv_id / "conversation.json").read_text())

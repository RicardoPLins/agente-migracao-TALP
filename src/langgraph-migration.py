...
def no_migrar_com_llm(estado: EstadoAgente, exemplos_treino: list[dict], prompt_sistema: str) -> dict:
    """
    Use LLM (Gemini) to intelligently migrate code.
...
Return ONLY the migrated Python code without any explanation or markdown.""")
        ...
        codigo_migrado = response.content
        ...
        mensagem = "🔄 Migração concluída com sucesso usando IA"
        
        return {
            "messages": [AIMessage(content=mensagem)],
            "codigo_migrado": codigo_migrado,
            "analise_agente": f"LLM migration using {len(exemplos_treino)} training examples",
            "status": "migrado"
        }
...
def decidir_proxima_etapa(estado: EstadoAgente) -> Literal["migrar", "validar", "fim"]:
    """
...
if __name__ == "__main__":
    ...
    # Example urllib code from dataset
    CODIGO_EXEMPLO = exemplos_treino[0]["before"]
    
    print("\n" + "=" * 80)
    print("📝 CÓDIGO URLLIB (ANTES):")
    print("=" * 80)
    print(CODIGO_EXEMPLO[:600] + "..." if len(CODIGO_EXEMPLO) > 600 else CODIGO_EXEMPLO)
